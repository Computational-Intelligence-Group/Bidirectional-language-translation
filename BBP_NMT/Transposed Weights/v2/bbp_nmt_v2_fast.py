"""
B-BP NMT v2 — Adigun-Kosko Framework
BPE Tokenization + Scaled Dot-Product Attention
================================================
Three fixes from v1:

  FIX 1 — BPE tokenization (sentencepiece, 8k vocab each language)
    v1 used word-level 10k vocab on WMT14 news → heavy <unk> flooding
    BPE splits unknown words into known subword pieces → near-zero OOV rate

  FIX 2 — Scaled dot-product attention over ALL encoder hidden states
    v1 compressed entire source into h_T → decoder collapsed after first token
    Attention lets decoder retrieve source info at each step → fixes collapse
    Also relieves orthogonality pressure on U (key for gradient conflict fix)

  FIX 3 — Input feeding (Luong et al. 2015)
    Context vector from step t fed as additional input to step t+1
    Decoder carries memory of what it already attended to

CORE B-BP CELL — IDENTICAL TO V1:
  Forward  (bbp=False): h_t = tanh(h_{t-1} @ U   + x_t + b)
  Backward (bbp=True):  h_t = tanh(h_{t-1} @ U^T + x_t + b)

ATTENTION MECHANISM — SCALED DOT PRODUCT (no extra scoring params):
  scores_i = (h_dec · h_enc_i) / sqrt(d)       shape: (B, T)
  alpha    = softmax(scores, masked at PAD)      shape: (B, T)
  context  = alpha @ H_enc                       shape: (B, d)

FOUR INFERENCE MODES (same as v1):
  EN→DE:           encode(U)   → attend(H_en) → decode(U)
  DE→EN:           encode(U^T) → attend(H_de) → decode(U^T)
  EN cycle recon:  encode(U)   → attend(H_en) → decode(U^T) → EN
  DE cycle recon:  encode(U^T) → attend(H_de) → decode(U)   → DE

Install (run once in Kaggle/Colab):
...
class BBPSeq2SeqAttn(nn.Module):
    ...
    def __init__(self, en_vocab: int, de_vocab: int,
        ...
        # Projects attended context before adding to h_t for output
        # This is the only new learned parameter in v2 (65k params)
        self.ctx_proj = nn.Linear(hid_dim, hid_dim, bias=False)

        # ── Separate enc/dec biases (direction-specific capacity) ─────────────
    ...
    def _enc_step(self, x: torch.Tensor, h: torch.Tensor, W: torch.Tensor):
    ...
    def encode(self, src: torch.Tensor, emb: nn.Embedding, bbp: bool):
        ...
        for t in range(T):
            ...
            h = self._enc_step(x, h, W)
            states.append(h)
        H = torch.stack(states)                     # (T, B, d)
        src_mask = (src == PAD_IDX).T               # (B, T) — True at PAD
        return h, H, src_mask
    ...
    def decode(self, tgt_in: torch.Tensor, h: torch.Tensor,
        ...
        out_emb : embedding module for target language
        ...
        # Initial context is zeros — no prior attention
        context = torch.zeros(B, self.hid_dim, device=tgt_in.device)
        logits  = []

        for t in range(T):
            # Input feeding: embed current token + previous context
            x = self.drop(out_emb(tgt_in[t])) + context         # (B, d)
            ...
            # Output: residual combination of recurrent state + projected context
            out   = h + self.ctx_proj(context)                   # (B, d)
            logit = self.drop(out) @ out_emb.weight.T            # (B, V)
            logits.append(logit)
    ...
    def forward_en_de(self, src_en, tgt_de_in):
        ...
        return self.decode(tgt_de_in, h, H, self.E_de, bbp=False, src_mask=mask)
    ...
    def forward_de_en(self, src_de, tgt_en_in):
        ...
        return self.decode(tgt_en_in, h, H, self.E_en, bbp=True, src_mask=mask)
    ...
    def translate(self, src: torch.Tensor, src_emb: nn.Embedding,
                  out_emb: nn.Embedding, bbp: bool, max_len: int = 50):
        """
        Greedy decode. Returns token ids (B, out_len).
        """
        self.eval()
        ...
        context = torch.zeros(B, self.hid_dim, device=src.device)
        output  = []

        for _ in range(max_len):
            x       = self.drop(out_emb(tok)) + context           # input feeding
            h       = self._dec_step(x, h, W)
            context = self.attention(h, H, mask)
            out     = h + self.ctx_proj(context)
            tok     = (out @ out_emb.weight.T).argmax(-1)          # (B,)
            output.append(tok)
            if (tok == EOS_IDX).all():
                break
    ...
    def translate_en_de(self, src_en, max_len=50):
        return self.translate(src_en, self.E_en, self.E_de, bbp=False, max_len=max_len)

    def translate_de_en(self, src_de, max_len=50):
        return self.translate(src_de, self.E_de, self.E_en, bbp=True,  max_len=max_len)
...
# ── 11. Training loop ─────────────────────────────────────────────────────────
def train_epoch(model, loader, opt, lam=LAM):
    model.train()
...
def main():
    ...
    other_p   = total - rec_p - emb_p - ctx_p

    print(f"\n{'='*70}")
    print(f"  B-BP NMT v2  |  BPE + Attention  |  Adigun-Kosko Framework")
    print(f"{'='*70}")
    print(f"  Total params         : {total:,}")
    print(f"  cell.U (recurrent)   : {rec_p:,}   ← used as U (fwd) and U^T (bwd)")
    print(f"  E_en + E_de (tied)   : {emb_p:,}   ← BPE 8k each")
    print(f"  ctx_proj (attn out)  : {ctx_p:,}   ← only new param vs v1")
    ...
    # ── Result directory ───────────────────────────────────────────────────────
    os.makedirs("bbp_v2_results", exist_ok=True)

    # CSV log flushed after every epoch — survives VM crashes
    csv_file = open("bbp_v2_results/training_log.csv", "w")
    csv_file.write("epoch,train_loss,nmt_loss,cyc_loss,val_loss,"
                   "bleu_en_de,bleu_de_en,elapsed_s\n")
    csv_file.flush()
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        csv_file.write(f"{ep},{tr:.4f},{nt:.4f},{cy:.4f},{vl:.4f},"
                       f"{b_en_de:.2f},{b_de_en:.2f},{elapsed:.0f}\n")
        csv_file.flush()
    ...
    best_ckpt = torch.load("bbp_v2_results/best.pt")
    ...
    print(f"\n{'='*70}")
    print(f"  FINAL TEST RESULTS — B-BP NMT v2")
    print(f"{'='*70}")
    ...
    with open("bbp_v2_results/summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print("\nAll results saved to bbp_v2_results/")

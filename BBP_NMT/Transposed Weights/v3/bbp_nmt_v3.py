"""
B-BP NMT v3 — Adigun-Kosko Framework
BPE + Attention + Stacked Encoder + Cycle Warmup
=================================================
Three fixes from v2 (flat val loss, zero BLEU for 8 epochs):

  FIX 1 — Learning rate: 3e-4 → 1e-3
    v2 gradients too small to coordinate attention + input feeding + B-BP cell.

  FIX 2 — Cycle consistency warmup schedule
    v2 applied λ=0.3 from epoch 1 — competed with translation signal.
    v3: ep1-5 λ=0.00 (translation only) | ep6-10 λ=0.05 | ep11+ λ=0.10

  FIX 3 — 2-layer stacked B-BP encoder (same U shared across both layers)
    Single-layer tanh encoder struggles over 40-token sequences.
    Layer 1 builds local features; Layer 2 builds compositional representations.

CORE B-BP CELL — IDENTICAL TO V1/V2:
  Forward  (bbp=False): h_t = tanh(h_{t-1} @ U   + x_t + b)
  Backward (bbp=True):  h_t = tanh(h_{t-1} @ U^T + x_t + b)

STACKED ENCODER:
  Layer 1: h1_t = BBPCell(embed(x_t), h1_{t-1}, bbp)
  Layer 2: h2_t = BBPCell(h1_t,       h2_{t-1}, bbp)   same U, same bbp flag
  Attention attends over H2. Decoder initialized from h2_T.

FOUR INFERENCE MODES (identical to v1/v2):
  EN→DE: encode(U)   → attend → decode(U)
  DE→EN: encode(U^T) → attend → decode(U^T)
  EN cycle: encode(U)   → attend → decode(U^T)
  DE cycle: encode(U^T) → attend → decode(U)

Install: pip install datasets sacrebleu sentencepiece
"""
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  B-BP NMT v3  |  BPE + Attention  |  Adigun-Kosko Framework")
    print(f"{'='*70}")
    ...
    # ── Result directory ───────────────────────────────────────────────────────
    os.makedirs("bbp_v3_results", exist_ok=True)

    # CSV log flushed after every epoch — survives VM crashes
    csv_file = open("bbp_v3_results/training_log.csv", "w")
    ...
    best_ckpt = torch.load("bbp_v3_results/best.pt")
    ...
    print(f"\n{'='*70}")
    print(f"  FINAL TEST RESULTS — B-BP NMT v3")
    print(f"{'='*70}")
    ...
    with open("bbp_v3_results/summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print("\nAll results saved to bbp_v3_results/")

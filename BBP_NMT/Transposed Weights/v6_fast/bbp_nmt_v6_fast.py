...
def compute_bleu_greedy(model, loader, direction, sp_hyp, sp_ref, max_batches=15):
    """
    Greedy decode BLEU for training monitoring — ~10x faster than beam.
    Scores are slightly lower than beam but track the same trend.
    Final test evaluation still uses full beam search.
    """
    model.eval()
    hyps, refs = [], []
    count = 0
    for en_b, de_b in loader:
        ...
        for i in range(B):
            ...
            for _ in range(50):
                ...
                ctx = model.attention(h, H, mask)
                out = h + model.ctx_proj(ctx)
                tok = out_proj(out).argmax(-1)
...
def main():
    ...
    best_val      = 1e9
    epoch_records = []
    os.makedirs("bbp_v6_fast_results", exist_ok=True)

    csv_file = open("bbp_v6_fast_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        }, f"bbp_v6_fast_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_v6_fast_results/checkpoint_ep{ep-3:02d}.pt"
    ...
    best_ckpt = torch.load("bbp_v6_fast_results/best.pt")
    ...
    with open("bbp_v6_fast_results/summary.json", "w") as f:
        json.dump({
            "model": "B-BP NMT v6",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_v6_fast_results/")

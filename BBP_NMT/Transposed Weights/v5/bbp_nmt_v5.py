"""
B-BP NMT v5 — Adigun-Kosko Framework
GRU Cell + Orthogonal Init + Ortho Regulariser + Beam Search + 30 Epochs
=========================================================================
Four improvements from v4 (EN->DE 4.10 / DE->EN 6.44 BLEU, test):

  CHANGE 1 — GRU-style B-BP cell
    v4 used vanilla tanh: h_t = tanh(h_{t-1} @ W + x_t + b)
    Vanishing gradient over long sequences explains why EN->DE lagged
    DE->EN throughout all 20 epochs.
    v5: GRU gates with U as the single shared recurrent matrix:
    ...
    W = U forward, W = U^T backward. B-BP claim fully preserved.

  CHANGE 2 — Orthogonal initialisation for U
    v4 initialised U randomly. For perfect B-BP inversion U should
    satisfy U^T @ U = I. Orthogonal init starts U at the ideal state.

  CHANGE 3 — Orthogonality regulariser
    Adds mu * ||U^T @ U - I||_F^2 to the training objective (mu=0.01).
    Directly enforces the Adigun-Kosko inversion claim during training.

  CHANGE 4 — Beam search decoding (k=4)
    v4 used greedy decoding. Beam search maintains top-k candidates
    and picks the best complete sequence. Typically +1-2 BLEU free.

  CHANGE 5 — 30 epochs
    v4 val loss was still decreasing at epoch 20. Model not converged.
...
def train_epoch(model, loader, opt, lam=0.0):
    ...
    for en_b, de_b in loader:
        ...
        ortho = model.ortho_loss()                   # CHANGE 3
        loss  = nmt + lam * cyc + MU_ORTHO * ortho  # CHANGE 3
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  B-BP NMT v5  |  GRU + Ortho Init + Ortho Reg + Beam  |  Adigun-Kosko")
    print(f"{'='*70}")
    ...
    print(f"  mu_ortho={MU_ORTHO}  cycle_warmup=True  input_feeding=False")
    ...
    best_val = 1e9
    epoch_records = []
    os.makedirs("bbp_v5_results", exist_ok=True)

    csv_file = open("bbp_v5_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        }, f"bbp_v5_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_v5_results/checkpoint_ep{ep-3:02d}.pt"
    ...
    best_ckpt = torch.load("bbp_v5_results/best.pt")
    ...
    with open("bbp_v5_results/summary.json", "w") as f:
        json.dump({
            "model": "B-BP NMT v5",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_v5_results/")

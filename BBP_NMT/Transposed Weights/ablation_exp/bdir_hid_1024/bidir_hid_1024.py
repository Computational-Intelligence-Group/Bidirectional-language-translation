"""
B-BP NMT Baseline — Bidirectinal Loss
======================================
...
# ── 11. Training loop — EXP 3: FULL B-BP OBJECTIVE ──────────────────────────
def train_epoch(model, loader, opt, lam=0.0):
    """
    Full B-BP objective: L_en_de + L_de_en + lambda*L_cyc + mu*L_ortho
    All four terms contribute to gradient updates.
    This establishes the v5 architecture ceiling at HID=1024.
    """
    model.train()
    ...
    for en_b, de_b in loader:
        ...
        loss  = nmt + lam * cyc + MU_ORTHO * ortho  # FULL B-BP OBJECTIVE
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  B-BP NMT Exp 3  |  Full B-BP Objective  |  HID=1024  |  Adigun-Kosko")
    print(f"{'='*70}")
    ...
    print(f"  loss=L_nmt + lam*L_cyc + mu*L_ortho  |  full B-BP objective")
    ...
    best_val = 1e9
    epoch_records = []
    os.makedirs("bbp_bidir_1024_results", exist_ok=True)

    csv_file = open("bbp_bidir_1024_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        }, f"bbp_bidir_1024_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_bidir_1024_results/checkpoint_ep{ep-3:02d}.pt"
        ...
        if vl < best_val:
            ...
            torch.save({
                ...
                "opt_state": opt.state_dict(), "val_loss": vl,
                "bleu_en_de": b_en_de, "bleu_de_en": b_de_en,
            }, "bbp_bidir_1024_results/best.pt")
    ...
    best_ckpt = torch.load("bbp_bidir_1024_results/best.pt")
    ...
    print(f"\n{'='*70}")
    print(f"  FINAL TEST RESULTS — Exp 3 (BiDir full, HID=1024)")
    print(f"{'='*70}")
    ...
    with open("bbp_bidir_1024_results/summary.json", "w") as f:
        json.dump({
            "model": "BBP-Exp3-BiDir-HID1024",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_bidir_1024_results/")

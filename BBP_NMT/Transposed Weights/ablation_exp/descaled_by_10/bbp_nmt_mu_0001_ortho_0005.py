"""
v5_full dataset run on GPU Cluster (2024-06-17)
"""
...
EPOCHS     = 30
ENC_LAYERS = 2
MU_ORTHO   = 0.0001        # CHANGE descaled by more 10
BEAM_SIZE  = 4
SEED       = 42
...
# Cycle warmup schedule — # CHANGE descaled by more 10
def get_lambda(epoch):
    if epoch <= 5:  return 0.00
    if epoch <= 10: return 0.0005
    return 0.0010
...
def main():
    ...
    best_val = 1e9
    epoch_records = []
    os.makedirs("bbp_v5_full_results", exist_ok=True)

    csv_file = open("bbp_v5_full_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        }, f"bbp_v5_full_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_v5_full_results/checkpoint_ep{ep-3:02d}.pt"
    ...
    best_ckpt = torch.load("bbp_v5_full_results/best.pt")
    ...
    with open("bbp_v5_full_results/summary.json", "w") as f:
        json.dump({
            "model": "B-BP NMT v5",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_v5_full_results/")

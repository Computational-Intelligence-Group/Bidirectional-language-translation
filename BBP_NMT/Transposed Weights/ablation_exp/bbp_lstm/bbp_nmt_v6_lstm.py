"""
B-BP NMT v6 LSTM
Bidirectional LSTM with single shared orthogonal matrix U
=======================================================================
...
KEY CHANGE FROM V6 GRU:
  - BBPGRUCell replaced with BBPLSTMCell
  - LSTM has 4 gates (f, i, o, g) vs GRU's 3 gates
  - Two hidden states: h (hidden) and c (cell state)
  - Cell state c provides separate long-term memory channel
  - Both h and c are propagated in encoder and decoder
  - Beam search tracks (score, tokens, h, c) instead of (score, tokens, h)
  - bi_proj_h and bi_proj_c combine forward/backward h and c separately
  - Expected +1 to +3 BLEU over GRU due to better long-range memory

UNCHANGED FROM V6 GRU:
  - Single shared matrix U — all 4 LSTM gates use same U/U^T
  - Orthogonal init and ortho regulariser mu=0.05
  - BiDir encoder: U forward, U^T backward
  - Scaled dot-product attention
  - Cycle warmup, beam search with length penalty
  - Full WMT14, 32k shared BPE, hid=512, batch=256
"""
...
# ── 6. B-BP LSTM Cell ─────────────────────────────────────────────────────────
class BBPLSTMCell(nn.Module):
    """
    LSTM-style B-BP cell. Single shared recurrent matrix U.
    Forward (bbp=False): W = U   | Backward (bbp=True): W = U^T

    All four gates use the same U / U^T as recurrent weight:
      f = σ(W_f·x + h@W)       forget gate
      i = σ(W_i·x + h@W)       input gate
      o = σ(W_o·x + h@W)       output gate
      g = tanh(W_g·x + h@W)    candidate cell

    Cell and hidden state update:
      c_t = f⊙c_{t-1} + i⊙g
      h_t = o⊙tanh(c_t)

    W_f, W_i, W_o, W_g are input-side projections — not subject to inversion.
    U is the sole recurrent weight — orthogonally initialised.

    Returns: (h_t, c_t) tuple — both hidden and cell states.
    """
    def __init__(self, dim):
        super().__init__()
        self.U   = nn.Parameter(torch.empty(dim, dim))
        nn.init.orthogonal_(self.U)
        self.W_f = nn.Linear(dim, dim, bias=True)   # forget gate
        self.W_i = nn.Linear(dim, dim, bias=True)   # input gate
        self.W_o = nn.Linear(dim, dim, bias=True)   # output gate
        self.W_g = nn.Linear(dim, dim, bias=True)   # candidate cell

    def forward(self, x, h, c, bbp=False):
        """
        x   : (B, dim) — input embedding
        h   : (B, dim) — previous hidden state
        c   : (B, dim) — previous cell state
        bbp : False=U (forward), True=U^T (backward)
        Returns: (h_t, c_t)
        """
        W   = self.U.T if bbp else self.U
        f   = torch.sigmoid(self.W_f(x) + h @ W)   # forget
        i   = torch.sigmoid(self.W_i(x) + h @ W)   # input
        o   = torch.sigmoid(self.W_o(x) + h @ W)   # output
        g   = torch.tanh(self.W_g(x) + h @ W)      # candidate
        c_t = f * c + i * g                          # cell update
        h_t = o * torch.tanh(c_t)                   # hidden update
        return h_t, c_t
...
class ScaledDotAttention(nn.Module):
    def forward(self, h_dec, H_enc, src_mask=None):
        ...
        if src_mask is not None:
            scores = scores.masked_fill(src_mask.bool(), -1e4)
...
# ── 8. B-BP LSTM Seq2Seq Model ────────────────────────────────────────────────
class BBPSeq2SeqAttn(nn.Module):
    """
    B-BP NMT v6 LSTM.

    Encoder — B-BP Bidirectional LSTM:
      Forward  (left->right):
        h_fwd_t, c_fwd_t = LSTM(embed(x_t), h_fwd_{t-1}, c_fwd_{t-1}, W=U)
      Backward (right->left):
        h_bwd_t, c_bwd_t = LSTM(embed(x_t), h_bwd_{t+1}, c_bwd_{t+1}, W=U^T)
      Combined h: H_t  = bi_proj_h( [h_fwd_t ; h_bwd_t] )  shape (T, B, d)
      Combined c: C_t  = bi_proj_c( [c_fwd_t ; c_bwd_t] )  shape (T, B, d)
      Decoder init:
        h_init = bi_proj_h( [h_fwd_T ; h_bwd_0] )
        c_init = bi_proj_c( [c_fwd_T ; c_bwd_0] )

    Decoder:
      x_t   = embed(y_{t-1})                     no input feeding
      h_t, c_t = LSTM(x_t, h_{t-1}, c_{t-1}, W) U or U^T
      ctx   = attention(h_t, H)
      o_t   = h_t + ctx_proj(ctx)
      logit = W_out_tgt(o_t)                      untied

    Four inference modes (same as GRU v6):
    ...
    Key difference from GRU v6:
      Beam search tracks (score, tokens, h, c) — 4-tuple instead of 3-tuple.
      encode() returns (h_init, c_init, H, src_mask) — 4 values instead of 3.
    """
    def __init__(self, vocab_size, hid_dim, dropout=0.3):
        super().__init__()
        self.hid_dim = hid_dim
        self.cell    = BBPLSTMCell(hid_dim)
        ...
        # Two bi_proj layers — one for h states, one for c states
        self.bi_proj_h = nn.Linear(hid_dim * 2, hid_dim, bias=False)
        self.bi_proj_c = nn.Linear(hid_dim * 2, hid_dim, bias=False)
    ...
    def ortho_loss(self):
        """Penalises U for deviating from U^T U = I."""
        U = self.cell.U
    ...
    def encode(self, src, emb):
        """
        B-BP Bidirectional LSTM Encoder.
        Both h and c states are propagated and combined separately.

        src : (T, B)
        emb : embedding module for source language
        Returns: h_init (B, d), c_init (B, d), H (T, B, d), src_mask (B, T)
        """
        T, B = src.shape

        # ── Forward pass: left to right, W=U ──────────────────────────────────
        h_fwd = torch.zeros(B, self.hid_dim, device=src.device)
        c_fwd = torch.zeros(B, self.hid_dim, device=src.device)
        H_fwd, C_fwd = [], []
        for t in range(T):
            x             = self.drop(emb(src[t]))
            h_fwd, c_fwd  = self.cell(x, h_fwd, c_fwd, bbp=False)  # W = U
            H_fwd.append(h_fwd)
            C_fwd.append(c_fwd)

        # ── Backward pass: right to left, W=U^T ───────────────────────────────
        h_bwd     = torch.zeros(B, self.hid_dim, device=src.device)
        c_bwd     = torch.zeros(B, self.hid_dim, device=src.device)
        H_bwd     = [None] * T
        C_bwd     = [None] * T
        for t in range(T - 1, -1, -1):
            x             = self.drop(emb(src[t]))
            h_bwd, c_bwd  = self.cell(x, h_bwd, c_bwd, bbp=True)   # W = U^T
            H_bwd[t]      = h_bwd
            C_bwd[t]      = c_bwd
        ...
        H_combined = []
        for t in range(T):
            H_combined.append(self.bi_proj_h(
                torch.cat([H_fwd[t], H_bwd[t]], dim=-1)
            ))
        H = torch.stack(H_combined)  # (T, B, d)

        # Decoder init from final states
        h_init = self.bi_proj_h(
            torch.cat([H_fwd[-1], H_bwd[0]], dim=-1)
        )  # (B, d)
        c_init = self.bi_proj_c(
            torch.cat([C_fwd[-1], C_bwd[0]], dim=-1)
        )  # (B, d)

        src_mask = (src == PAD_IDX).T  # (B, T)
        return h_init, c_init, H, src_mask

    def decode(self, tgt_in, h, c, H_enc, out_emb, out_proj, bbp, src_mask=None):
        """
        LSTM Decoder.
        tgt_in  : (T_tgt, B)
        h       : (B, d)    decoder h_init
        c       : (B, d)    decoder c_init
        ...
        logits = []
        for t in range(T):
            x        = self.drop(out_emb(tgt_in[t]))
            h, c     = self.cell(x, h, c, bbp=bbp)
    ...
    def forward_en_de(self, src_en, tgt_de_in):
        ...
        h, c, H, mask = self.encode(src_en, self.E_en)
        return self.decode(tgt_de_in, h, c, H, self.E_de, self.W_out_de,
                           bbp=False, src_mask=mask)

    def forward_de_en(self, src_de, tgt_en_in):
        """DE->EN: bidir encode DE, decode with U^T -> English"""
        h, c, H, mask = self.encode(src_de, self.E_de)
        return self.decode(tgt_en_in, h, c, H, self.E_en, self.W_out_en,
                           bbp=True, src_mask=mask)

    def cycle_en(self, src_en, tgt_en_in):
        """EN cycle: encode EN, decode with U^T -> reconstruct EN"""
        h, c, H, mask = self.encode(src_en, self.E_en)
        return self.decode(tgt_en_in, h, c, H, self.E_en, self.W_out_en,
                           bbp=True, src_mask=mask)

    def cycle_de(self, src_de, tgt_de_in):
        """DE cycle: encode DE, decode with U -> reconstruct DE"""
        h, c, H, mask = self.encode(src_de, self.E_de)
        return self.decode(tgt_de_in, h, c, H, self.E_de, self.W_out_de,
                           bbp=False, src_mask=mask)
    ...
    def beam_decode(self, src, src_emb, out_emb, out_proj, bbp,
        ...
        """
        Beam search with length penalty for LSTM.
        Beams track (score, tokens, h, c) — both LSTM states.
        """
        self.eval()
        h_init, c_init, H, mask = self.encode(src, src_emb)

        # beams: (raw_log_score, token_list, h, c)
        beams     = [(0.0, [SOS_IDX], h_init, c_init)]
        completed = []
        ...
        for _ in range(max_len):
            ...
            for score, tokens, h, c in beams:
                ...
                for s, t in zip(topk_scores.tolist(), topk_toks.tolist()):
                    all_cands.append((score + s, tokens + [t], h, c))
...
def main():
    ...
    gate_p   = (sum(p.numel() for p in model.cell.W_f.parameters()) +
                sum(p.numel() for p in model.cell.W_i.parameters()) +
                sum(p.numel() for p in model.cell.W_o.parameters()) +
                sum(p.numel() for p in model.cell.W_g.parameters()))
    biproj_p = model.bi_proj_h.weight.numel() + model.bi_proj_c.weight.numel()
    ...
    print(f"\n{'='*72}")
    print(f"  B-BP NMT v6 LSTM  |  BiDir LSTM + hid=512 + 32k BPE  | ")
    print(f"{'='*72}")
    print(f"  Total params              : {total:,}")
    print(f"  cell.U (recurrent)        : {rec_p:,}   <- shared U, ortho init")
    print(f"  LSTM gates (W_f,W_i,W_o,W_g): {gate_p:,}   <- input projections (4 gates)")
    print(f"  bi_proj_h + bi_proj_c     : {biproj_p:,}   <- bidir h and c combiners")
    ...
    print(f"  cell=LSTM  encoder=B-BP-BiDir  input_feeding=False  vocab=32k-shared")
    ...
    print(f"  LSTM cell: 4 gates (f,i,o,g) all use same U/U^T — B-BP inversion preserved")
    ...
    best_val      = 1e9
    epoch_records = []
    os.makedirs("bbp_v6_lstm_results", exist_ok=True)

    csv_file = open("bbp_v6_lstm_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        }, f"bbp_v6_lstm_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_v6_lstm_results/checkpoint_ep{ep-3:02d}.pt"
    ...
    best_ckpt = torch.load("bbp_v6_lstm_results/best.pt")
    ...
    print(f"\n{'='*72}")
    print(f"  FINAL TEST RESULTS — B-BP NMT v6 LSTM")
    print(f"{'='*72}")
    ...
    with open("bbp_v6_lstm_results/summary.json", "w") as f:
        json.dump({
            "model": "B-BP NMT v6 LSTM",
            "cell": "LSTM",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_v6_lstm_results/")

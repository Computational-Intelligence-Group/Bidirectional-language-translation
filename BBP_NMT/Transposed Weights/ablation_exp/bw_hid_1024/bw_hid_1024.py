"""
B-BP NMT Experiment 2 — Backward Loss Only, HID=1024
=====================================================
Only L_de_en (backward DE->EN cross-entropy) is backpropagated.
L_en_de, L_cyc, L_ortho are computed and logged every epoch
but do NOT contribute to gradient updates.

This answers: what does U produce as a zero-shot forward translator
when U is trained purely on backward translation via U^T with no
forward supervision?

Symmetric counterpart to Exp 1 (forward only).
...
# ── 11. Training loop — EXP 2: BACKWARD LOSS ONLY ───────────────────────────
def train_epoch(model, loader, opt, lam=0.0):
    """
    Exp 2: only L_de_en (backward DE->EN) is backpropagated.
    L_en_de, L_cyc, L_ortho are computed and logged but NOT in the loss.
    This shows what U produces as a zero-shot forward translator when U
    is trained purely on backward DE->EN translation via U^T.
    """
    model.train()
    ...
    for en_b, de_b in loader:
        ...
        # EXP 2: only backward DE->EN loss is backpropagated
        loss = l_de_en
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  B-BP NMT Exp 2  |  Backward Loss Only  |  HID=1024  |  Adigun-Kosko")
    print(f"{'='*70}")
    ...
    print(f"  EXP 2: loss=L_de_en only  |  fw/cyc/ortho logged but NOT backpropagated")
    ...
    best_val = 1e9
    epoch_records = []
    os.makedirs("bbp_baseline_bw_results", exist_ok=True)

    csv_file = open("bbp_baseline_bw_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        }, f"bbp_baseline_bw_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_baseline_bw_results/checkpoint_ep{ep-3:02d}.pt"
        ...
        if vl < best_val:
            ...
            torch.save({
                ...
                "opt_state": opt.state_dict(), "val_loss": vl,
                "bleu_en_de": b_en_de, "bleu_de_en": b_de_en,
            }, "bbp_baseline_bw_results/best.pt")
    ...
    best_ckpt = torch.load("bbp_baseline_bw_results/best.pt")
    ...
    print(f"\n{'='*70}")
    print(f"  FINAL TEST RESULTS — B-BP NMT Exp 2 (BW only, HID=1024)")
    print(f"{'='*70}")
    ...
    with open("bbp_baseline_bw_results/summary.json", "w") as f:
        json.dump({
            "model": "B-BP NMT Exp2 BW-only HID=1024",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_baseline_bw_results/")

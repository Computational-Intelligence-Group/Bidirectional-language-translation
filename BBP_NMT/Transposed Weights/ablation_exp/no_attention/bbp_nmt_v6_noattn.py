"""
B-BP NMT v6 No-Attention Ablation
===========================================================
Ablation study: identical to v6_fast but with attention removed.

PURPOSE:
  Measure the contribution of the attention mechanism to BLEU.
  The decoder uses only the encoder summary vector h_init at each step.
  No attention over encoder memory H. No ctx_proj.

  Comparison target (v6 full with attention):
    EN->DE: ~20-24 BLEU  |  DE->EN: ~25-28 BLEU  (projected epoch 30)

  Expected result (no attention):
    EN->DE: ~8-12 BLEU   |  DE->EN: ~14-18 BLEU
    Gap = attention contribution to BLEU

ARCHITECTURE DIFFERENCE FROM V6:
  v6 decoder step:
    h_t   = GRU(embed(y_{t-1}), h_{t-1}, W)
    ctx   = attention(h_t, H)           <- REMOVED in this ablation
    out   = h_t + ctx_proj(ctx)         <- REPLACED with out = h_t
    logit = W_out(out)

  This ablation decoder step:
    h_t   = GRU(embed(y_{t-1}), h_{t-1}, W)
    logit = W_out(h_t)                  <- direct from GRU hidden state

EVERYTHING ELSE IDENTICAL TO V6_FAST:
  - B-BP BiDir Encoder (U left->right, U^T right->left)
  - hid=512, 32k shared BPE, full WMT14
  - FP16 mixed precision, greedy BLEU every 3 epochs
  - Cycle warmup: ep1-5 lam=0.00 | ep6-10 lam=0.05 | ep11+ lam=0.10
  - Ortho regulariser mu=0.05
  - Beam search k=5 with length penalty=0.6 at test evaluation
  - torch.compile, pre-allocated tensors
"""
...
# ── 7. Attention removed — ablation study ────────────────────────────────────
# ScaledDotAttention is intentionally absent in this ablation.
# The decoder uses only the encoder summary vector h_init at each step.
# This isolates the attention mechanism's contribution to BLEU.

# ── 8. B-BP Seq2Seq No-Attention Model ───────────────────────────────────────
class BBPSeq2SeqAttn(nn.Module):
    """
    B-BP NMT v6 — No Attention Ablation.

    Encoder — B-BP Bidirectional (identical to v6):
    ...
    Decoder — NO ATTENTION (ablation):
      x_t   = embed(y_{t-1})             no input feeding
      h_t   = GRU(x_t, h_{t-1}, bbp)    U or U^T
      logit = W_out_tgt(h_t)             direct from hidden state, no attention

    The encoder memory H is computed but NOT used by the decoder.
    This is the ablation condition — h_init is the only encoder signal
    available to the decoder at every step.

    Four inference modes (unchanged):
    ...
    def __init__(self, vocab_size, hid_dim, dropout=0.3):
        ...
        # BiDir encoder combiner (2*hid -> hid)
        self.bi_proj  = nn.Linear(hid_dim * 2, hid_dim, bias=False)
        # NOTE: ctx_proj and attention are intentionally absent (ablation)
    ...
    def encode(self, src, emb):
        ...
        H_fwd = torch.zeros(T, B, self.hid_dim, device=src.device)  # pre-alloc
        for t in range(T):
        ...
        H_bwd = torch.zeros(T, B, self.hid_dim, device=src.device)  # pre-alloc
        ...
        # Stack fwd+bwd along last dim: (T, B, 2d) -> bi_proj -> (T, B, d)
        H_cat = torch.cat([H_fwd, H_bwd], dim=-1)     # (T, B, 2d)
        H = self.bi_proj(H_cat)                        # (T, B, d) — batched

        # Decoder init: combine final fwd and final bwd states
        h_init = self.bi_proj(
    ...
    def decode(self, tgt_in, h, H_enc, out_emb, out_proj, bbp, src_mask=None):
        """
        Decoder — NO ATTENTION (ablation).
        H_enc and src_mask are accepted for API compatibility but not used.
        The decoder relies solely on h_init from the encoder at every step.

        tgt_in  : (T_tgt, B)
        h       : (B, d)    decoder init from encoder — only encoder signal
        H_enc   : (T, B, d) encoder memory — NOT used in this ablation
        out_emb : target-side embedding
        ...
        for t in range(T):
            ...
            # ABLATION: no attention, no ctx_proj
            # logit directly from GRU hidden state
            logit = self.drop(out_proj(h))
            logits.append(logit)
    ...
    def beam_decode(self, src, src_emb, out_emb, out_proj, bbp,
        ...
        """
        Beam search with length penalty — NO ATTENTION (ablation).
        Decoder uses only h_init as encoder signal at every step.
        src: (T, 1) — single sentence
        """
        self.eval()
        h_init, H, mask = self.encode(src, src_emb)
        # H and mask are not used — ablation condition
        ...
        for _ in range(max_len):
            ...
            for score, tokens, h in beams:
                ...
                h   = self.cell(x, h, bbp=bbp)
                # ABLATION: no attention, output directly from h
                log_probs = F.log_softmax(out_proj(h), dim=-1)
...
# ── 10a. Greedy BLEU — fast, used during training monitoring ──────────────────
@torch.no_grad()
def compute_bleu_greedy(model, loader, direction, sp_hyp, sp_ref, max_batches=15):
    """
    Greedy decode BLEU for training monitoring — NO ATTENTION (ablation).
    ~10x faster than beam. Final test evaluation uses full beam search.
    """
    model.eval()
    hyps, refs = [], []
    count = 0
    for en_b, de_b in loader:
        ...
        for i in range(B):
            if direction == "en_de":
                ...
                bbp      = False
                out_emb  = model.E_de
                out_proj = model.W_out_de
            else:
                src      = de_b[:, i:i+1]
                ref_ids  = en_b[:, i].tolist()
                bbp      = True
                out_emb  = model.E_en
                out_proj = model.W_out_en

            h, H, mask = model.encode(src, model.E_en if direction == "en_de" else model.E_de)
            # H and mask not used — ablation condition
            tok  = torch.tensor([SOS_IDX], device=src.device)
            pred = []
            for _ in range(50):
                x   = model.drop(out_emb(tok))
                h   = model.cell(x, h, bbp=bbp)
                # ABLATION: no attention, output directly from h
                tok = out_proj(h).argmax(-1)
                if tok.item() == EOS_IDX: break
                if tok.item() not in (PAD_IDX, SOS_IDX):
                    pred.append(tok.item())

            clean_r = [t for t in ref_ids if t not in (PAD_IDX, SOS_IDX, EOS_IDX)]
            hyps.append(sp_hyp.decode(pred))
            refs.append([sp_ref.decode(clean_r)])
        count += 1
    return sacrebleu.corpus_bleu(hyps, list(zip(*refs))).score

# ── 10b. BLEU with beam search — used for final test evaluation ───────────────
...
# ── 11. Training loop with FP16 mixed precision ───────────────────────────────
scaler = GradScaler()   # AMP gradient scaler — handles FP16 overflow

def train_epoch(model, loader, opt, lam=0.0):
    """
    Mixed precision training (FP16 forward, FP32 optimizer step).
    ~30-50% faster on A100 with no impact on final BLEU.
    """
    model.train()
    ...
    for en_b, de_b in loader:
        ...
        scaler.step(opt)                              # FP32 optimizer step
        scaler.update()
...
def main():
    ...
    # torch.compile fuses operations for ~10-20% speedup (PyTorch 2.0+)
    try:
    ...
    # ctx_proj absent — ablation
    other_p  = total - rec_p - gate_p - biproj_p - emb_p - out_p

    print(f"\n{'='*72}")
    print(f"  B-BP NMT v6 NO-ATTENTION ABLATION  |  ")
    print(f"{'='*72}")
    print(f"  Total params              : {total:,}")
    print(f"  cell.U (recurrent)        : {rec_p:,}   <- shared U, ortho init")
    print(f"  GRU gates (W_z,W_r,W_n)  : {gate_p:,}   <- input projections")
    print(f"  bi_proj (2*hid -> hid)    : {biproj_p:,}   <- bidir combiner")
    print(f"  E_en + E_de (input emb)   : {emb_p:,}   <- 32k shared vocab")
    print(f"  W_out_en + W_out_de       : {out_p:,}   <- untied output")
    print(f"  ctx_proj                  : REMOVED   <- attention ablation")
    print(f"  attention                 : REMOVED   <- attention ablation")
    ...
    print(f"  encoder=B-BP-BiDir  decoder=NO-ATTENTION  vocab=32k-shared")
    print(f"  ABLATION: attention and ctx_proj removed to measure attention contribution")
    ...
    best_val      = 1e9
    epoch_records = []
    os.makedirs("bbp_v6_noattn_results", exist_ok=True)

    csv_file = open("bbp_v6_noattn_results/training_log.csv", "w")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        sched.step(vl)
        # Greedy BLEU for training monitoring (fast) — beam only at test
        if ep % 3 == 0 or ep == 1 or ep == EPOCHS:
            b_en_de = compute_bleu_greedy(model, val_loader, "en_de", sp_de, sp_de)
            b_de_en = compute_bleu_greedy(model, val_loader, "de_en", sp_en, sp_en)
        else:
            b_en_de = -1.0   # not computed this epoch
            b_de_en = -1.0
        ...
        }, f"bbp_v6_noattn_results/checkpoint_ep{ep:02d}.pt")

        old = f"bbp_v6_noattn_results/checkpoint_ep{ep-3:02d}.pt"
    ...
    best_ckpt = torch.load("bbp_v6_noattn_results/best.pt")
    ...
    print(f"\n{'='*72}")
    print(f"  FINAL TEST RESULTS — B-BP NMT v6 NO-ATTENTION ABLATION")
    print(f"{'='*72}")
    ...
    print(f"\n  Ablation Comparison (projected v6 full vs no-attention)")
    print(f"  {'Metric':<32} {'V6-NoAttn':>10} {'V6-Full':>10}")
    print(f"  {'-'*54}")
    print(f"  {'EN->DE BLEU (test)':<32} {test_en_de:>10.2f} {'~20-24':>10}")
    print(f"  {'DE->EN BLEU (test)':<32} {test_de_en:>10.2f} {'~25-28':>10}")
    print(f"  {'Avg BLEU (test)':<32} {(test_en_de+test_de_en)/2:>10.2f} {'~22-26':>10}")
    print(f"  {'Attention':<32} {'NO':>10} {'YES':>10}")
    print(f"  {'Encoder':<32} {'BiDir':>10} {'BiDir':>10}")

    with open("bbp_v6_noattn_results/summary.json", "w") as f:
        json.dump({
            "model": "B-BP NMT v6 No-Attention Ablation",
            "ablation": "attention_removed",
            "total_params": total,
            "recurrent_U": rec_p,
    ...
    print("\nAll results saved to bbp_v6_noattn_results/")
